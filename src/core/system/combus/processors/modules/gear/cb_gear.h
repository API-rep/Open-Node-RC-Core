 ...
 *
 * @details Five CbProc functions wrapping the gear FSM + transmission model:
 *
 ...
 *
 *   2. `gear_ratio_fn` — RPM × gearRatio[gear]/1000 → wheel-speed RPM (RPM domain).
 *      Scales RPM magnitude by the gear ratio (‰) of the current gear.
 *      GEAR=0 → passthrough (neutral / idle sentinel).
 *      inCh = GEAR (analog): active gear from GEAR channel.
 *      cfg = GearProcCfg*, state = nullptr.
 *
 *   3. `gear_subgear_cap_fn` — sub-gear speed cap (RPM domain).
 ...
 *      cfg = GearProcCfg*, state = nullptr.
 *
 *   4. `gear_dir_fn` — unsigned RPM magnitude → bipolar ComBus.
 ...
 *      cfg = GearProcCfg*, state = nullptr.
 *
 *   5. `gear_dyn_ramp_fn` — gear → rampTime bridge.
