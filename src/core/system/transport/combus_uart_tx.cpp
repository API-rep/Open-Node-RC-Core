...
void combus_uart_tx_init( HardwareSerial* serial,
    ...
    uartTx.port     = serial;
    uartTx.envId    = envId;
    uartTx.nAnalog  = nAnalog;
    uartTx.nDigital = nDigital;
    uartTx.periodMs = 1000u / txHz;
...
void combus_uart_tx_update(const ComBus* bus, bool failSafe) {
    ...
    if (!uartTx.port || uartTx.periodMs == 0u || !bus) { return; }

      // --- Timer gate ---
    uint32_t now = millis();
    if ((now - uartTx.lastTxMs) < uartTx.periodMs) { return; }
    uartTx.lastTxMs = now;
    ...
    if (frameLen == 0u) { return; }
      ...
      // --- Send ---
    uartTx.port->write(frame, frameLen);
    uartTx.seq++;

    output_log_dbg("[UART_TX] seq=%u  len=%u  rl=%d  flags=0x%02X\n",
             (unsigned)(uartTx.seq - 1u),
             (unsigned)frameLen,
             (int)bus->runLevel,
