 ...
 * @brief UART adapter for the generic TransportIface.
 *
 * @details Initializes a HardwareSerial port once and returns a claimed
 * TransportIface* that encapsulates the UART read/write/available calls.
 *
 ...
 * Typical use:
 * @code
 *   TransportIface* t = uart_transport_init(&Serial2, 115200, TX_PIN, RX_PIN, "combus");
 *   combus_tx_init(t, ...);   // protocol layer receives the same pointer
 *   combus_rx_init(t, ...);   // both share the same claimed port
 * @endcode
 *****************************************************************************/
#pragma once
...
/**
 * @brief Initialize a UART port and return a claimed TransportIface*.
 *
 ...
 *
 * @return Pointer to a ready-to-use TransportIface, or nullptr on failure.
 */
